 ...
 *
 * The four states form a strict order: draft < checked < published < expired,
 ...
 *   published → expired   : checkedAt + publishedAt kept; expiredAt set
 *   expired   → published : checkedAt + publishedAt kept; expiredAt kept (set-if-absent)
 *   expired   → draft     : all three cleared
 *
 * Because each forward move only sets a timestamp when it is absent, round
 * trips through a higher state and back (e.g. published → checked → published,
...
export function applyStatusTransition(
  ...
  if (newStatus === "draft") {
    ...
    // publishedAt / expiredAt are left untouched, so they survive a move back to checked.
