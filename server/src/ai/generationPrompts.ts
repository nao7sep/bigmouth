...
export const LEGACY_DEFAULT_GENERATION_PREAMBLE =
...
export const LEGACY_DEFAULT_GENERATION_PROMPTS: Record<string, string> = {
  title:
...
export function normalizeGenerationPromptsData(
  data: GenerationPromptsData
): { data: GenerationPromptsData; changed: boolean } {
  let changed = false;

  const normalizedPreamble =
    data.preamble === undefined || data.preamble === LEGACY_DEFAULT_GENERATION_PREAMBLE
      ? DEFAULT_GENERATION_PREAMBLE
      : data.preamble;
  if (normalizedPreamble !== data.preamble) {
    changed = true;
  }

  const sourcePrompts =
    data.prompts && typeof data.prompts === "object" ? data.prompts : {};
  const normalizedPrompts: Record<string, string> = { ...sourcePrompts };

  for (const [key, nextDefault] of Object.entries(DEFAULT_GENERATION_PROMPTS)) {
    const current = normalizedPrompts[key];
    const legacy = LEGACY_DEFAULT_GENERATION_PROMPTS[key];
    if (typeof current !== "string") {
      normalizedPrompts[key] = nextDefault;
      changed = true;
      continue;
    }
    if (current === legacy) {
