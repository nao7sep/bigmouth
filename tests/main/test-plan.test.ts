...
const repositoryReaders = ["tests/main/ipc/workspaces.test.ts", "tests/main/services/storagePaths.test.ts"];
...
describe("the default test plan", () => {
  ...
  it("typechecks and runs related and repository-reading tests for a TypeScript change", () => {
    expect(plan(["src/main/core/services/postStore.ts"])).toEqual({
      typecheck: true,
      vitest: ["src/main/core/services/postStore.ts", ...repositoryReaders],
      live: false,
    });
  });

  it("runs the related and repository-reading tests without the type check for a stylesheet change", () => {
    expect(plan(["src/renderer/src/App.css"])).toEqual({
      typecheck: false,
      vitest: ["src/renderer/src/App.css", ...repositoryReaders],
      live: false,
    });
  });
  ...
  it("typechecks a live test but never runs the live lane", () => {
    expect(plan(["tests/live/main/ipc/ai-handlers.test.ts"])).toMatchObject({ typecheck: true, live: false });
  });
