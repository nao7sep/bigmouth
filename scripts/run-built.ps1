...
try {
    ...
    & node $runtimeHelper stop electron "BigMouth" "BigMouth"
    if ($LASTEXITCODE -ne 0) { throw "Could not replace the existing BigMouth runtime." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "BigMouth did not start." }
}
catch {
    Write-Host ""
