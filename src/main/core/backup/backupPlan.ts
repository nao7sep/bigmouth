 ...
 */
import type { BackupCandidate, BackupIndex, BackupIndexEntry } from "./backupTypes.js";

/**
 * The modification-time equality window, in milliseconds. Two seconds absorbs FAT/exFAT's 2-second mtime
 * granularity (a workspace may live on a USB stick); it costs nothing in missed edits because the recorded
...
/** The latest entry per archive path. `archivedAt` is a fixed-width `yyyymmdd-hhmmss-fff-utc` (or, for
 *  entries written before milliseconds were adopted, `yyyymmdd-hhmmss-utc`) stamp, so ordinal string
 *  comparison is chronological within either form. */
